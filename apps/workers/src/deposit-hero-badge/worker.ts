...
import { DepositHeroBadgeController } from './controller'
...
export const DepositHeroBadgeWorker = async (
  connection: ConnectionOptions,
  dependencies: {
    logger: AppLogger
    controller: DepositHeroBadgeController
  ...
  const worker = new Worker<BatchJob<DepositHeroBadgeJob>>(
    QueueName.DepositHeroBadge,
    async (job) => {
      ...
      logger.info({
        method: 'DepositHeroBadgeWorker.process',
        batchId: job.data.id,
      ...
      try {
        ...
        logger.error({
          method: 'DepositHeroBadgeWorker.error',
          id: job.data.id,
          error
        })
